...
function App() {
  ...
  // Update the randomize function with better protection
  const randomizeSettings = useDebounce(() => {
    ...
    try {
      ...
      if (randomSettings.pattern) {
        ...
        newSettings.pattern = patterns[Math.floor(Math.random() * patterns.length)].id;
      }
      
      if (randomSettings.gridSize) {
        const sizes = [30, 40, 50, 60];
        newSettings.gridSize = sizes[Math.floor(Math.random() * sizes.length)];
      }
      
      if (randomSettings.pValue) {
        const pValues = [2, 2.5, 3, 3.5, 4, 4.5, 5, 6];
        newSettings.pValue = pValues[Math.floor(Math.random() * pValues.length)];
      }
      
      if (randomSettings.renderStyle) {
        const styles = Object.values(RENDER_STYLES);
        newSettings.renderStyle = styles[Math.floor(Math.random() * styles.length)];
      }
        ...
        <div className="panel-content">
          <div className="controls-grid">
            ...
            <div className="control-section">
              ...
              <select className="select-control" value={gridSize} onChange={(e) => setGridSize(Number(e.target.value))}>
